...
| Control Plane | PENDING_RECOMMENDATIONS, COMPLETION_RECOMMENDATIONS, ATTENTION_SUMMARY, HANDOFF_CONTEXT |
...
When the active phase prompt or system prompt references them, the executor hydrates the control-plane variables from project state before resolution: pending recommendations are summarized for prompt injection, recommendations generated by the current task run are exposed through `COMPLETION_RECOMMENDATIONS`, blocked and failed tasks are compacted into `ATTENTION_SUMMARY`, and the current task gets a bounded `HANDOFF_CONTEXT`. Unused variables stay empty without touching the backend; referenced variables fail the phase if their source data cannot be loaded. Treat this as a package invariant, not an optimization detail: eager whole-project prompt enrichment on every phase is a regression in orc, not acceptable convenience.
