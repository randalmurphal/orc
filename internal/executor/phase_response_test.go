...
func TestParsePhaseSpecificResponse(t *testing.T) {
	...
	tests := []struct {
		...
		// Review round 1 - ReviewFindingsSchema (status: complete/blocked)
		{
			...
			phaseID:     "review",
			reviewRound: 1,
			content:     `{"needs_changes": false, "round": 1, "summary": "Review complete", "issues": []}`,
			wantStatus:  PhaseStatusComplete,
		...
		},
		{
			...
			phaseID:     "review",
			reviewRound: 1,
			content:     `{"needs_changes": false, "round": 1, "summary": "Found issues", "issues": [{"severity": "high", "description": "Bug found"}]}`,
			wantStatus:  PhaseStatusComplete,
		...
		},
		{
			...
			phaseID:     "review",
			reviewRound: 1,
			content:     `{"needs_changes": true, "round": 1, "summary": "No implementation exists to review", "issues": []}`,
			wantStatus:  PhaseStatusBlocked,
