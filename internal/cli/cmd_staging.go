...
Example:
  orc staging status`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Show configuration
			fmt.Fprintln(out, "Staging Branch Configuration")
			fmt.Fprintln(out, strings.Repeat("-", 40))

			if cfg.Developer.StagingBranch == "" {
				fmt.Fprintln(out, "Branch:  (not configured)")
				fmt.Fprintln(out, "Enabled: false")
				fmt.Fprintln(out)
				fmt.Fprintln(out, "To configure a staging branch:")
				fmt.Fprintln(out, "  orc staging set dev/yourname")
				fmt.Fprintln(out, "  orc staging enable")
				return nil
			}
			...
			if cfg.Developer.AutoSyncAfter > 0 {
				fmt.Fprintf(out, "Auto-sync: after %d tasks\n", cfg.Developer.AutoSyncAfter)
			}
			...
			fmt.Fprintf(out, "Sync target: %s\n", targetBranch)

			// Try to get git status
			projectRoot, err := config.FindProjectRoot()
			if err != nil {
				fmt.Fprintln(out)
				fmt.Fprintln(out, "(Not in a git repository)")
				return nil
			}
			...
			fmt.Fprintln(out)
			fmt.Fprintln(out, "Git Status")
			fmt.Fprintln(out, strings.Repeat("-", 40))

			if !exists {
				fmt.Fprintf(out, "Branch %s does not exist yet.\n", cfg.Developer.StagingBranch)
				fmt.Fprintln(out, "It will be created when the first task runs.")
				return nil
			}

			fmt.Fprintf(out, "Branch %s exists.\n", cfg.Developer.StagingBranch)
			fmt.Fprintln(out)
			fmt.Fprintln(out, "To sync changes to main:")
			fmt.Fprintln(out, "  orc staging sync")
...
Example:
  ...
  orc staging sync --draft  # Create as draft PR`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			ghClient, err := github.NewClient(projectRoot)
			if err != nil {
...
Generated by `+"`orc staging sync`", targetBranch),
			...
			fmt.Fprintf(cmd.OutOrStdout(), "Creating PR: %s → %s...\n", cfg.Developer.StagingBranch, targetBranch)
			pr, err := ghClient.CreatePR(cmd.Context(), opts)
			if err != nil {
...
func setStagingConfig(cmd *cobra.Command, key, value string) error {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Set developer.%s = %s in ~/.orc/config.yaml\n", key, value)
	return nil
}
