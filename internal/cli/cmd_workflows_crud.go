...
Examples:
  ...
  orc workflow new quick-impl --from small --description "Fast implementation"`,
	...
	cmd.Flags().String("from", "", "Clone from existing workflow")
	cmd.Flags().String("description", "", "Workflow description")

	return cmd
}

func newWorkflowEditCmd() *cobra.Command {
	cmd := &cobra.Command{
...
Examples:
  ...
  orc workflow clone implement-medium my-medium -f        # Overwrite if exists`,
	...
	cmd.Flags().StringP("level", "l", "project", "Target level: personal, local, shared, project")
	cmd.Flags().BoolP("force", "f", false, "Overwrite if exists")
...
Examples:
  ...
  orc workflow sync --force     # Force update all (including embedded)`,
	...
	cmd.Flags().Bool("force", false, "Force update all workflows including embedded")
