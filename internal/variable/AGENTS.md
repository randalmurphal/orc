...
| Control Plane | `PENDING_RECOMMENDATIONS` (all pending recommendation summary, truncated to 8 KiB), `COMPLETION_RECOMMENDATIONS` (pending recommendations created by the current task run, truncated to 8 KiB), `ATTENTION_SUMMARY` (blocked and failed task summary, truncated to 4 KiB), `HANDOFF_CONTEXT` (current task handoff pack, truncated to 16 KiB) |
...
| Scratchpad | `PREV_SCRATCHPAD` (notes from prior phases), `RETRY_SCRATCHPAD` (notes from prior attempt of current phase) |
