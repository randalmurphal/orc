...
ALTER TABLE tasks ADD COLUMN pr_mergeable BOOLEAN DEFAULT TRUE;
...
ALTER TABLE tasks ADD COLUMN pr_merged BOOLEAN DEFAULT FALSE;
ALTER TABLE tasks ADD COLUMN pr_merged_at TIMESTAMP WITH TIME ZONE;
ALTER TABLE tasks ADD COLUMN pr_merge_commit_sha TEXT;
ALTER TABLE tasks ADD COLUMN pr_target_branch TEXT;
ALTER TABLE tasks ADD COLUMN pr_last_checked_at TIMESTAMP WITH TIME ZONE;

-- Testing requirements (JSON: {"unit": true, "e2e": false, "visual": false})
ALTER TABLE tasks ADD COLUMN testing_requirements TEXT;

-- UI testing flag
ALTER TABLE tasks ADD COLUMN requires_ui_testing BOOLEAN DEFAULT FALSE;
...
ALTER TABLE tasks ADD COLUMN executor_started_at TIMESTAMP WITH TIME ZONE;
ALTER TABLE tasks ADD COLUMN last_heartbeat TIMESTAMP WITH TIME ZONE;
...
ALTER TABLE tasks ADD COLUMN session_created_at TIMESTAMP WITH TIME ZONE;
ALTER TABLE tasks ADD COLUMN session_last_activity TIMESTAMP WITH TIME ZONE;
...
ALTER TABLE phases ADD COLUMN interrupted_at TIMESTAMP WITH TIME ZONE;
...
-- Note: FTS5 virtual tables and triggers are omitted for PostgreSQL.
-- PostgreSQL will use tsvector-based FTS in a separate task.
...
CREATE TABLE IF NOT EXISTS task_attachments (
    ...
    content_type TEXT,
    size_bytes INTEGER,
    data BYTEA,  -- Store file content directly in DB
...
INSERT INTO sync_state (id, site_id, sync_enabled)
VALUES (1, replace(gen_random_uuid()::text, '-', ''), FALSE)
