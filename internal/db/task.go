...
type Task struct {
	...
	CurrentPhase     string
	CurrentIteration int // Iteration number for current phase (1-based)
	Branch           string
	WorktreePath string
...
func (p *ProjectDB) SaveTask(t *Task) error {
	...
	`, t.ID, t.Title, t.Description, t.Weight, t.Status, stateStatus, t.CurrentPhase, t.CurrentIteration, t.Branch, t.WorktreePath,
...
func (p *ProjectDB) GetTask(id string) (*Task, error) {
	row := p.QueryRow(`
		SELECT id, title, description, weight, status, state_status, current_phase, current_iteration, branch, worktree_path, queue, priority, category, initiative_id, created_at, started_at, completed_at, updated_at, total_cost_usd, metadata, retry_context, quality, executor_pid, executor_hostname, executor_started_at, last_heartbeat, is_automation
		FROM tasks WHERE id = ?
	`, id)
...
func scanTask(row *sql.Row) (*Task, error) {
	...
	var currentIteration sql.NullInt64
	...
	if err := row.Scan(&t.ID, &t.Title, &description, &t.Weight, &t.Status, &stateStatus, &currentPhase, &currentIteration, &branch, &worktreePath,
	...
	if currentIteration.Valid {
		t.CurrentIteration = int(currentIteration.Int64)
	}
	if branch.Valid {
...
func scanTaskRows(rows *sql.Rows) (*Task, error) {
	...
	if err := rows.Scan(&t.ID, &t.Title, &description, &t.Weight, &t.Status, &stateStatus, &currentPhase, &currentIteration, &branch, &worktreePath,
		&queue, &priority, &category, &initiativeID, &createdAt, &startedAt, &completedAt, &updatedAt, &t.TotalCostUSD, &metadata, &retryContext, &quality,
		&executorPID, &executorHostname, &executorStartedAt, &lastHeartbeat, &isAutomation); err != nil {
		return nil, err
	}
