...
// ListInitiativeNotes returns all notes for an initiative.
func (s *initiativeServer) ListInitiativeNotes(
	ctx context.Context,
	req *connect.Request[orcv1.ListInitiativeNotesRequest],
) (*connect.Response[orcv1.ListInitiativeNotesResponse], error) {
	if req.Msg.InitiativeId == "" {
		return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("initiative_id is required"))
	}
	...
	if req.Msg.NoteType != nil && *req.Msg.NoteType != "" {
		notes, err = backend.GetInitiativeNotesByType(req.Msg.InitiativeId, *req.Msg.NoteType)
	} else {
		notes, err = backend.GetInitiativeNotes(req.Msg.InitiativeId)
	}
	if err != nil {
		return nil, connect.NewError(connect.CodeInternal, fmt.Errorf("get notes: %w", err))
	}

	// Convert to proto
	protoNotes := make([]*orcv1.InitiativeNote, len(notes))
	for i, n := range notes {
		protoNotes[i] = initiative.NoteToProto(&n)
	}

	return connect.NewResponse(&orcv1.ListInitiativeNotesResponse{
		Notes: protoNotes,
	}), nil
}
...
func (s *initiativeServer) ListTaskGeneratedNotes(
	ctx context.Context,
	req *connect.Request[orcv1.ListTaskGeneratedNotesRequest],
) (*connect.Response[orcv1.ListTaskGeneratedNotesResponse], error) {
	if req.Msg.TaskId == "" {
		return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("task_id is required"))
	}
	...
	notes, err := backend.GetInitiativeNotesBySourceTask(req.Msg.TaskId)
	if err != nil {
		return nil, connect.NewError(connect.CodeInternal, fmt.Errorf("get notes: %w", err))
	}
	...
	return connect.NewResponse(&orcv1.ListTaskGeneratedNotesResponse{
		Notes: protoNotes,
	}), nil
}
