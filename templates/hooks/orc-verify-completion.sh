...
# Use task ID for marker uniqueness, fall back to parent PID (Claude Code process)
# Note: $PPID is consistent across hook invocations within the same Claude Code session,
# unlike $$ which gives a new PID for each bash subprocess
MARKER_FILE="/tmp/orc-verify-completion-${ORC_TASK_ID:-$PPID}"
