...
describe("TaskHeader", () => {
  const createTask = (
    overrides: Partial<Omit<Task, "$typeName" | "$unknown">> = {},
  ): Task => {
...
describe("TaskHeader handoff actions", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("renders HandoffActions with the task source wiring", () => {
    render(
      <TooltipProvider delayDuration={0}>
        <MemoryRouter>
          <TaskHeader
...
describe("Running Status Badge (TASK-312)", () => {
  ...
  const createPlan = (phases: string[], currentPhase?: string) => ({
    ...
    phases: phases.map((name, idx) => ({
      ...
      name,
      status:
        phases.indexOf(name) < phases.indexOf(currentPhase ?? "")
